package peak.cuckoo.game.behavior.render.gpu
{
   ...
   public class AGALMiniAssembler
   {
      ...
      private static function init() : void
      {
         ...
         OPMAP["ted"] = new OpCode("ted",3,38,0x20 | 8 | 0x0100);
      ...
      public function assemble(param1:String, param2:String, param3:uint = 1, param4:Boolean = false) : ByteArray
      {
         ...
         var _loc43_:uint = uint(getTimer());
         ...
         agalcode.writeByte(_loc44_ ? 1 : 0);
         initregmap(param3,param4);
         var _loc47_:Array = param2.replace(/[\f\n\r\v]+/g,"\n").split("\n");
         var _loc37_:int = 0;
         var _loc13_:int = int(_loc47_.length);
         _loc36_ = 0;
         while(_loc36_ < _loc13_ && _error == "")
         {
            _loc12_ = new String(_loc47_[_loc36_]);
            _loc12_ = _loc12_.replace(REGEXP_OUTER_SPACES,"");
            _loc39_ = _loc12_.search("//");
            if(_loc39_ != -1)
            {
               _loc12_ = _loc12_.slice(0,_loc39_);
            }
            _loc5_ = _loc12_.search(/<.*>/g);
            if(_loc5_ != -1)
            {
               _loc6_ = _loc12_.slice(_loc5_).match(/([\w\.\-\+]+)/gi);
               _loc12_ = _loc12_.slice(0,_loc5_);
            }
            _loc17_ = _loc12_.match(/^\w{3}/gi);
            if(!_loc17_)
            {
               if(_loc12_.length >= 3)
               {
                  trace("warning: bad line " + _loc36_ + ": " + _loc47_[_loc36_]);
               }
            }
            else
            {
               _loc21_ = OPMAP[_loc17_[0]];
               if(debugEnabled)
               {
                  trace(_loc21_);
               }
               ...
               {
                  _loc12_ = _loc12_.slice(_loc12_.search(_loc21_.name) + _loc21_.name.length);
                  if(_loc21_.flags & 0x0100 && param3 < 2)
                  {
                     _error = "error: opcode requires version 2.";
                     break;
                  }
                  if(_loc21_.flags & 0x40 && _loc44_)
                  {
                     _error = "error: opcode is only allowed in vertex programs.";
                     break;
                  }
                  if(_loc21_.flags & 0x20 && !_loc44_)
                  {
                  ...
                  if(verbose)
                  {
                     trace("emit opcode=" + _loc21_);
                  }
                  agalcode.writeUnsignedInt(_loc21_.emitCode);
                  if(++_loc37_ > 2048)
                  {
                     _error = "error: too many opcodes. maximum is 2048.";
                     break;
                  }
                  _loc30_ = _loc12_.match(/vc\[([vof][acostdip]?)(\d*)?(\.[xyzw](\+\d{1,3})?)?\](\.[xyzw]{1,4})?|([vof][acostdip]?)(\d*)?(\.[xyzw]{1,4})?/gi);
                  if(!_loc30_ || _loc30_.length != _loc21_.numRegister)
                  {
                     _error = "error: wrong number of operands. found " + _loc30_.length + " but expected " + _loc21_.numRegister + ".";
                     break;
                  }
                  _loc38_ = false;
                  _loc26_ = 160;
                  _loc9_ = _loc30_.length;
                  _loc32_ = 0;
                  while(_loc32_ < _loc9_)
                  {
                     _loc19_ = false;
                     _loc29_ = _loc30_[_loc32_].match(/\[.*\]/gi);
                     if(_loc29_ && _loc29_.length > 0)
                     {
                        _loc30_[_loc32_] = _loc30_[_loc32_].replace(_loc29_[0],"0");
                        if(verbose)
                        {
                           trace("IS REL");
                        }
                     ...
                     _loc7_ = _loc30_[_loc32_].match(/^\b[A-Za-z]{1,2}/gi);
                     if(!_loc7_)
                     {
                        _error = "error: could not parse operand " + _loc32_ + " (" + _loc30_[_loc32_] + ").";
                        _loc38_ = true;
                        break;
                     }
                     _loc16_ = REGMAP[_loc7_[0]];
                     if(debugEnabled)
                     {
                        trace(_loc16_);
                     }
                     if(_loc16_ == null)
                     {
                        _error = "error: could not find register name for operand " + _loc32_ + " (" + _loc30_[_loc32_] + ").";
                        _loc38_ = true;
                        break;
                     }
                     if(_loc44_)
                     {
                        if(!(_loc16_.flags & 0x20))
                        {
                           _error = "error: register operand " + _loc32_ + " (" + _loc30_[_loc32_] + ") only allowed in vertex programs.";
                           _loc38_ = true;
                           break;
                        }
                        if(_loc19_)
                        {
                           _error = "error: register operand " + _loc32_ + " (" + _loc30_[_loc32_] + ") relative adressing not allowed in fragment programs.";
                           _loc38_ = true;
                           break;
                        }
                     }
                     else if(!(_loc16_.flags & 0x40))
                     {
                        _error = "error: register operand " + _loc32_ + " (" + _loc30_[_loc32_] + ") only allowed in fragment programs.";
                        _loc38_ = true;
                        break;
                     }
                     _loc30_[_loc32_] = _loc30_[_loc32_].slice(_loc30_[_loc32_].search(_loc16_.name) + _loc16_.name.length);
                     _loc28_ = _loc19_ ? _loc29_[0].match(/\d+/) : _loc30_[_loc32_].match(/\d+/);
                     _loc35_ = 0;
                     if(_loc28_)
                     {
                        _loc35_ = uint(_loc28_[0]);
                     }
                     if(_loc16_.range < _loc35_)
                     {
                        _error = "error: register operand " + _loc32_ + " (" + _loc30_[_loc32_] + ") index exceeds limit of " + (_loc16_.range + 1) + ".";
                        _loc38_ = true;
                        break;
                     }
                     _loc48_ = 0;
                     _loc49_ = _loc30_[_loc32_].match(/(\.[xyzw]{1,4})/);
                     _loc14_ = _loc32_ == 0 && !(_loc21_.flags & 0x80);
                     _loc10_ = _loc32_ == 2 && _loc21_.flags & 8;
                     _loc42_ = 0;
                     _loc34_ = 0;
                     _loc15_ = 0;
                     if(_loc14_ && _loc19_)
                     ...
                     {
                        _loc48_ = 0;
                        _loc11_ = uint(_loc49_[0].length);
                        _loc33_ = 1;
                        while(_loc33_ < _loc11_)
                        {
                           _loc31_ = uint(_loc49_[0].charCodeAt(_loc33_) - "x".charCodeAt(0));
                           if(_loc31_ > 2)
                           {
                              _loc31_ = 3;
                           }
                     ...
                     {
                        _loc48_ = uint(_loc14_ ? 15 : 228);
                     }
                     if(_loc19_)
                     {
                        _loc23_ = _loc29_[0].match(/[A-Za-z]{1,2}/gi);
                        _loc46_ = REGMAP[_loc23_[0]];
                        if(_loc46_ == null)
                        {
                        ...
                        _loc42_ = _loc46_.emitCode;
                        _loc8_ = _loc29_[0].match(/(\.[xyzw]{1,1})/);
                        if(_loc8_.length == 0)
                        {
                        ...
                        _loc34_ = uint(_loc8_[0].charCodeAt(1) - "x".charCodeAt(0));
                        if(_loc34_ > 2)
                        {
                           _loc34_ = 3;
                        }
                        _loc24_ = _loc29_[0].match(/\+\d{1,3}/gi);
                        if(_loc24_.length > 0)
                        {
                           _loc15_ = int(_loc24_[0]);
                        }
                        if(_loc15_ < 0 || _loc15_ > 255)
                        {
                           _error = "error: index offset " + _loc15_ + " out of bounds. [0..255]";
                           _loc38_ = true;
                           break;
                        }
                        if(verbose)
                        {
                           trace("RELATIVE: type=" + _loc42_ + "==" + _loc23_[0] + " sel=" + _loc34_ + "==" + _loc8_[0] + " idx=" + _loc35_ + " offset=" + _loc15_);
                        }
                     }
                     if(verbose)
                     {
                        trace("  emit argcode=" + _loc16_ + "[" + _loc35_ + "][" + _loc48_ + "]");
                     }
                     if(_loc14_)
                     {
                        agalcode.writeShort(_loc35_);
                        agalcode.writeByte(_loc48_);
                        agalcode.writeByte(_loc16_.emitCode);
                        _loc26_ -= 32;
                     }
                     else if(_loc10_)
                     {
                        ...
                        _loc41_ = 5;
                        _loc25_ = uint(_loc6_ == null ? 0 : _loc6_.length);
                        _loc22_ = 0;
                        _loc33_ = 0;
                        while(_loc33_ < _loc25_)
                        {
                           if(verbose)
                           {
                              trace("    opt: " + _loc6_[_loc33_]);
                           }
                           _loc40_ = SAMPLEMAP[_loc6_[_loc33_]];
                           if(_loc40_ == null)
                           {
                              _loc22_ = Number(_loc6_[_loc33_]);
                              if(verbose)
                              {
                                 trace("    bias: " + _loc22_);
                              }
                           ...
                           else
                           {
                              if(_loc40_.flag != 16)
                              {
                                 _loc41_ &= ~(15 << _loc40_.flag);
                              }
                              _loc41_ |= _loc40_.mask << _loc40_.flag;
                           }
                           _loc33_++;
                        }
                        agalcode.writeShort(_loc35_);
                        agalcode.writeByte(int(_loc22_ * 8));
                        agalcode.writeByte(0);
                        agalcode.writeUnsignedInt(_loc41_);
                        if(verbose)
                        {
                           trace("    bits: " + (_loc41_ - 5));
                        }
                        _loc26_ -= 64;
                     }
                     else
                     {
                        ...
                        agalcode.writeShort(_loc35_);
                        agalcode.writeByte(_loc15_);
                        agalcode.writeByte(_loc48_);
                        agalcode.writeByte(_loc16_.emitCode);
                        agalcode.writeByte(_loc42_);
                        agalcode.writeShort(_loc19_ ? _loc34_ | 0x8000 : 0);
                        _loc26_ -= 64;
                     }
                     _loc32_++;
                  }
                  _loc32_ = 0;
                  while(_loc32_ < _loc26_)
                  {
                     agalcode.writeByte(0);
                     _loc32_ += 8;
                  }
         ...
         if(_error != "")
         {
            _error += "\n  at line " + _loc36_ + " " + _loc47_[_loc36_];
            agalcode.length = 0;
            trace(_error);
         }
         if(debugEnabled)
         {
            _loc18_ = "generated bytecode:";
            _loc20_ = agalcode.length;
            _loc27_ = 0;
            while(_loc27_ < _loc20_)
            {
               ...
               _loc45_ = agalcode[_loc27_].toString(16);
               if(_loc45_.length < 2)
               {
                  _loc45_ = "0" + _loc45_;
               }
         ...
         if(verbose)
         {
            trace("AGALMiniAssembler.assemble time: " + (getTimer() - _loc43_) / 1000 + "s");
         }
         return agalcode;
      }
      ...
      private function initregmap(param1:uint, param2:Boolean) : void
      {
         ...
         REGMAP["vc"] = new Register("vc","vertex constant",1,param2 ? 1024 : (param1 == 1 ? 127 : 250),0x40 | 2);
         REGMAP["vt"] = new Register("vt","vertex temporary",2,param2 ? 1024 : (param1 == 1 ? 7 : 27),0x40 | 1 | 2);
         REGMAP["vo"] = new Register("vo","vertex output",3,param2 ? 1024 : 0,0x40 | 1);
         REGMAP["vi"] = new Register("vi","varying",4,param2 ? 1024 : (param1 == 1 ? 7 : 11),0x40 | 0x20 | 2 | 1);
         REGMAP["fc"] = new Register("fc","fragment constant",1,param2 ? 1024 : (param1 == 1 ? 27 : 63),0x20 | 2);
         REGMAP["ft"] = new Register("ft","fragment temporary",2,param2 ? 1024 : (param1 == 1 ? 7 : 27),0x20 | 1 | 2);
