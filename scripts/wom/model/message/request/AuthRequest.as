package wom.model.message.request
{
   import peak.messaging.AbstractOutgoingMessage;
   import peak.messaging.OutgoingMessage;
   
   public class AuthRequest extends AbstractOutgoingMessage implements OutgoingMessage
   {
      
      private var _gameId:String;
      
      private var _fbid:String;
      
      private var _iosUDID:String;
      
      private var _iosIFV:String;
      
      private var _andID:String;
      
      private var _friendIds:Array;
      
      private var _timestamp:Number;
      
      private var _locale:String;
      
      private var _signature:String;
      
      private var _axess:String;
      
      private var _sdp:String;
      
      private var _sdt:String;
      
      private var _kid:String;
      
      public function AuthRequest(param1:String, param2:String, param3:String, param4:String, param5:String, param6:String, param7:Array, param8:Number, param9:String, param10:String, param11:String = null)
      {
         super();
         _gameId = param1;
         _fbid = param2;
         _iosUDID = param3;
         _iosIFV = param4;
         _andID = param5;
         _kid = param6;
         _friendIds = param7;
         _timestamp = param8;
         _locale = param9;
         _signature = param10;
         _axess = param11;
         _sdp = null;
         _sdt = null;
      }
      
      override public function serialize() : Object
      {
         var _loc1_:Object = {
            "gameUid":_gameId,
            "fbid":_fbid,
            "ios_udid":_iosUDID,
            "ios_ifv":_iosIFV,
            "and_id":_andID,
            "kid":_kid,
            "friendIds":_friendIds,
            "ts":_timestamp,
            "loc":_locale,
            "sig":_signature
         };
         if(_axess != null)
         {
            _loc1_.axess = _axess;
         }
         if(_sdp != null && _sdt != null)
         {
            _loc1_.sdp = _sdp;
            _loc1_.sdt = _sdt;
         }
         return _loc1_;
      }
      
      public function set sdp(param1:String) : void
      {
         _sdp = param1;
      }
      
      public function set sdt(param1:String) : void
      {
         _sdt = param1;
      }
   }
}

