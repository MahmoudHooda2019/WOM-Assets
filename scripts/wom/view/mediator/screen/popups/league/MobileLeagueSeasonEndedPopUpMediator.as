package wom.view.mediator.screen.popups.league
{
   import starling.events.Event;
   import wom.view.mediator.util.MobileGenericWindowMediator;
   import wom.view.screen.popups.league.MobileLeagueSeasonEndedPopUp;
   
   public class MobileLeagueSeasonEndedPopUpMediator extends MobileGenericWindowMediator
   {
      
      [Inject]
      public var view:MobileLeagueSeasonEndedPopUp;
      
      public function MobileLeagueSeasonEndedPopUpMediator()
      {
         super();
      }
      
      override public function onRegister() : void
      {
         super.onRegister();
         eventMap.mapStarlingListener(view.okButton,"triggered",onOkButtonClicked,Event);
      }
      
      protected function onOkButtonClicked(param1:Event) : void
      {
         closeWindow();
      }
   }
}

