package starling.display
{
   ...
   public class QuadBatch extends DisplayObject
   {
            ...
            for each(_loc8_ in [true,false])
            {
               for each(_loc9_ in [true,false])
               {
                  for each(_loc10_ in _loc6_)
                  {
                     ...
                     {
                        _loc12_ = RenderSupport.getTextureLookupFlags(_loc11_,_loc9_,_loc8_,_loc10_);
                        _loc1_.registerProgram(getImageProgramName(_loc5_,_loc9_,_loc8_,_loc11_,_loc10_),_loc2_.assemble(Context3DProgramType.VERTEX,_loc3_),_loc2_.assemble(Context3DProgramType.FRAGMENT,_loc4_.replace("<???>",_loc12_)));
                     }
                  }
               }
            }
      ...
      public function transformQuadAt(param1:int, param2:Matrix) : void
      {
