package starling.display.materials
{
   ...
   public class StandardMaterial implements IMaterial
   {
      ...
      public function dispose(param1:Boolean = false) : void
      {
         ...
         if(param1)
         {
            for each(var _loc2_ in _textures)
            {
               _loc2_.dispose();
            }
         }
      ...
      public function set color(param1:uint) : void
      {
         ...
         colorVector[1] = ((_color & 0xFF00) >> 8) / 255;
         colorVector[2] = (_color & 0xFF) / 255;
      }
      
      public function drawTriangles(param1:Context3D, param2:Matrix3D, param3:VertexBuffer3D, param4:IndexBuffer3D, param5:Number = 1) : void
      {
         var _loc6_:int = 0;
         ...
         while(_loc6_ < 8)
         {
            param1.setTextureAt(_loc6_,_loc6_ < _textures.length ? _textures[_loc6_].base : null);
            _loc6_++;
         }
