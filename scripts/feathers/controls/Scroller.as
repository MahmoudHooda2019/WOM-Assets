package feathers.controls
{
   ...
   public class Scroller extends FeathersControl
   {
      ...
      public function set scrollerProperties(param1:Object) : void
      {
         var _loc3_:PropertyProxy = null;
         if(this._scrollerProperties == param1)
         {
            return;
         }
         if(!param1)
         ...
         this._scrollerProperties = PropertyProxy(param1);
         if(this._scrollerProperties)
         {
      ...
      override protected function draw() : void
      {
         ...
         do
         {
            ...
            if(_loc2_ || _loc6_ || _loc5_ || _loc11_ || _loc7_)
            {
               ...
               this.refreshScrollValues(_loc2_);
            }
            if(++_loc1_ >= 10)
            {
               break;
      ...
      protected function refreshScrollValues(param1:Boolean) : void
      {
      ...
      {
         var _loc1_:Boolean = this._scrollBarDisplayMode == "fixed";
         var _loc2_:int = this.numChildren;
         if(this.verticalScrollBar)
         {
            this.verticalScrollBar.visible = !_loc1_ || this._hasVerticalScrollBar;
            this.setChildIndex(DisplayObject(this.verticalScrollBar),_loc2_ - 1);
         }
         if(this.horizontalScrollBar)
         {
            ...
            if(this.verticalScrollBar)
            {
               this.setChildIndex(DisplayObject(this.horizontalScrollBar),_loc2_ - 2);
            }
            else
            {
               this.setChildIndex(DisplayObject(this.horizontalScrollBar),_loc2_ - 1);
            }
         }
         if(this.currentBackgroundSkin)
         {
      ...
      protected function calculateViewPortOffsets(param1:Boolean = false, param2:Boolean = false) : void
      {
         ...
         if(this._scrollBarDisplayMode == "fixed")
         {
            if(this.horizontalScrollBar)
            {
               _loc3_ = param2 ? this.actualWidth : this.explicitWidth;
               if(param1 || this._horizontalScrollPolicy == "on" || (this._viewPort.width > _loc3_ || this._viewPort.width > this._maxWidth) && this._horizontalScrollPolicy != "off")
               {
            ...
            if(this.verticalScrollBar)
            {
               _loc4_ = param2 ? this.actualHeight : this.explicitHeight;
               if(param1 || this._verticalScrollPolicy == "on" || (this._viewPort.height > _loc4_ || this._viewPort.height > this._maxHeight) && this._verticalScrollPolicy != "off")
               {
      ...
      protected function updateHorizontalScrollFromTouchPosition(param1:Number) : void
      {
         ...
         if(_loc2_ < this._minHorizontalScrollPosition)
         {
            ...
            {
               _loc2_ -= (_loc2_ - this._minHorizontalScrollPosition) * (1 - this._elasticity);
            }
            else
            {
         ...
         else if(_loc2_ > this._maxHorizontalScrollPosition)
         {
            if(this._hasElasticEdges)
            {
               _loc2_ -= (_loc2_ - this._maxHorizontalScrollPosition) * (1 - this._elasticity);
            }
            else
            {
      ...
      protected function updateVerticalScrollFromTouchPosition(param1:Number) : void
      {
         ...
         if(_loc2_ < this._minVerticalScrollPosition)
         {
            if(this._hasElasticEdges)
            {
               _loc2_ -= (_loc2_ - this._minVerticalScrollPosition) * (1 - this._elasticity);
            }
            else
            {
         ...
         else if(_loc2_ > this._maxVerticalScrollPosition)
         {
            if(this._hasElasticEdges)
            {
               _loc2_ -= (_loc2_ - this._maxVerticalScrollPosition) * (1 - this._elasticity);
            }
            else
            {
      ...
      protected function scrollerProperties_onChange(param1:PropertyProxy, param2:String) : void
      {
         var _loc3_:Object = null;
         if(this.hasOwnProperty(param2))
         {
            _loc3_ = this._scrollerProperties[param2];
            this[param2] = _loc3_;
         }
      }
      ...
      protected function scroller_touchHandler(param1:TouchEvent) : void
      {
         ...
         this._previousTouchY = this._startTouchY = this._currentTouchY = _loc3_;
      ...
      protected function stage_touchHandler(param1:TouchEvent) : void
      {
         ...
         else if(_loc3_.phase == "ended")
         {
            ...
            if(!_loc7_ && this._isDraggingHorizontally)
            {
               ...
               _loc4_ = int(this._previousVelocityX.length);
               _loc5_ = 2.33;
               _loc9_ = 0;
               while(_loc9_ < _loc4_)
               {
                  _loc2_ = VELOCITY_WEIGHTS[_loc9_];
                  _loc6_ += this._previousVelocityX.shift() * _loc2_;
                  _loc5_ += _loc2_;
                  _loc9_++;
               }
            ...
            if(!_loc8_ && this._isDraggingVertically)
            {
               ...
               _loc4_ = int(this._previousVelocityY.length);
               _loc5_ = 2.33;
               _loc9_ = 0;
               while(_loc9_ < _loc4_)
               {
                  _loc2_ = VELOCITY_WEIGHTS[_loc9_];
                  _loc6_ += this._previousVelocityY.shift() * _loc2_;
                  _loc5_ += _loc2_;
                  _loc9_++;
               }
      ...
      protected function nativeStage_mouseWheelHandler(param1:MouseEvent) : void
      {
         ...
         HELPER_POINT.x = (param1.stageX - _loc2_.x) / Starling.contentScaleFactor;
         HELPER_POINT.y = (param1.stageY - _loc2_.y) / Starling.contentScaleFactor;
         if(this.contains(this.stage.hitTest(HELPER_POINT,true)))
         {
            ...
            this.revealVerticalScrollBar();
            _loc3_ = this._verticalScrollPosition - param1.delta * this.actualVerticalScrollStep;
            if(_loc3_ < this._minVerticalScrollPosition)
            {
               _loc3_ = this._minVerticalScrollPosition;
            }
